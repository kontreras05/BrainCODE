...
import { Clock, Coffee, Infinity as InfinityIcon, Play, Repeat, Timer } from "lucide-react";
    ...
    <div className="bc-setup">
      {!mode && (
        <div className="bc-mode-row">
          <button className="bc-mode-btn pri" onClick={() => setMode("pom")}>
            <Timer size={16} strokeWidth={STROKE} />
            <span>Pomodoro</span>
          </button>
          <button className="bc-mode-btn" onClick={() => onStart({ mode: "freeflow" })}>
            <InfinityIcon size={16} strokeWidth={STROKE} />
            <span>Libre</span>
          </button>
        </div>
      )}
